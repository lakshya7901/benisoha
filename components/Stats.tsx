...
export default function Stats() {
  return (
    <section className="py-20 bg-charcoal relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-gold-500 to-transparent" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-4">
          {stats.map((stat, i) => (
            <div key={stat.label} className="text-center group">
              ...
              {i < stats.length - 1 && (
                <div className="hidden md:block absolute right-0 top-1/2 -translate-y-1/2 w-[1px] h-12 bg-gold-800" />
              )}
            </div>
          ))}
        </div>
